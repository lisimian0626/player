...
public class CGEFrameRenderer {
    ...
    public void setFilterWidthConfig(final String config) {
        if(mNativeAddress != 0)
            nativeSetFilterWithConfig(mNativeAddress, config, true);
    }

    //initialize the filters width config string
    public void setFilterWidthConfig(final String config, final boolean shouldCleanOld) {
        if(mNativeAddress != 0)
            nativeSetFilterWithConfig(mNativeAddress, config, shouldCleanOld);
    }
    ...
    //用于加入自定义滤镜
    public void setNativeFilter(long nativeFilter, boolean clean) {
        nativeSetFilterWithAddr(mNativeAddress, nativeFilter, clean);
    }

    //用于加入自定义滤镜
    public void setNativeFilter(long nativeFilter) {
        nativeSetFilterWithAddr(mNativeAddress, nativeFilter, true);
    }
...
//    protected native void nativeSetFilterWithConfig(long holder, String config);
    protected native void nativeSetFilterWithConfig(long holder, String config, boolean shouldCleanOld);
    ...
    protected native void nativeSetFilterWithAddr(long holder, long filter, boolean clean);
}
