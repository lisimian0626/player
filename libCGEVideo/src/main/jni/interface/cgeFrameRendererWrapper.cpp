 ...
 */
JNIEXPORT void JNICALL Java_org_wysaid_nativePort_CGEFrameRenderer_nativeSetFilterWithConfig
		(JNIEnv *env, jobject obj, jlong addr, jstring config, jboolean shouldCleanOld)
{
	static CGETexLoadArg texLoadArg;
	texLoadArg.env = env;
	texLoadArg.cls = env->FindClass("org/wysaid/nativePort/CGENativeLibrary");
	...
	CGEFrameRenderer* renderer = (CGEFrameRenderer*)addr;
	const char* configStr = env->GetStringUTFChars(config, 0);
	renderer->setFilterWithConfig(configStr, cgeGlobalTextureLoadFunc, &texLoadArg, shouldCleanOld);
	env->ReleaseStringUTFChars(config, configStr);
}
...
JNIEXPORT void JNICALL Java_org_wysaid_nativePort_CGEFrameRenderer_nativeSetFilterWithAddr
  (JNIEnv *, jobject, jlong addr, jlong filter, jboolean clean)
{
	CGEFrameRenderer* renderer = (CGEFrameRenderer*)addr;
	renderer->setFilter((CGEImageFilterInterfaceAbstract*)filter, clean);
}
