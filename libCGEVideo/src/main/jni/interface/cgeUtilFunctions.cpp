...
extern "C"
{
	...
	CGETextureResult* cgeLoadTexture(JNIEnv* env, jobject bmp) {
		...
		char *bmpPtr;
		if (AndroidBitmap_lockPixels(env, bmp, (void**) &bmpPtr) >= 0) {
			glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, bmpInfo.width, bmpInfo.height, 0, GL_RGBA, GL_UNSIGNED_BYTE, bmpPtr);
		}
		...
		CGETextureResult *textureResult = new CGETextureResult();
		textureResult->width = bmpInfo.width;
		textureResult->height = bmpInfo.height;
		textureResult->texID = textureId;
