...
{
	cgePrintGLInfo();
	CGETexLoadArg *arg = new CGETexLoadArg();
	arg->env = env;
	arg->cls = env->FindClass("org/wysaid/nativePort/CGENativeLibrary");
    CGEFrameRecorder* recorder = new CGEFrameRecorder(arg);
...
JNIEXPORT jboolean JNICALL Java_org_wysaid_nativePort_CGEFrameRecorder_nativeSetLyrics
        (JNIEnv *env, jobject, jlong addr, jobject bmp)
{
    CGEFrameRecorder* recorder = (CGEFrameRecorder*)addr;

	jclass cls = env->FindClass("org/wysaid/nativePort/CGENativeLibrary");
    return recorder->setLyrics(env, cls, bmp);
}
