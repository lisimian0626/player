...
//        c->strict_std_compliance = -2;
            ...
            c->sample_rate      = audioSampleRate;
            c->channel_layout   = AV_CH_LAYOUT_STEREO;//AV_CH_LAYOUT_MONO;AV_CH_LAYOUT_STEREO
...
namespace CGE
{
	...
	bool CGEVideoEncoderMP4::init(const char* filename, int fps, int width, int height, bool hasAudio, int bitRate, int audioSampleRate, AVDictionary* options, const char* rotation)
	{
		...
		m_videoPacketBufferSize = CGE::CGE_MAX(1024 * 1024, width * height * 8);
		m_videoPacketBuffer = (unsigned char*)av_malloc(m_videoPacketBufferSize);

		if(m_hasAudio)
		{
			m_audioPacketBufferSize = 1024 * 1024;
	...
	bool CGEVideoEncoderMP4::_openAudio()
	{
		...
		if (c->sample_fmt != AV_SAMPLE_FMT_S16)
		{
			...
			* converted input samples */
            ...
            CGE_LOG_INFO("samplelinesize=%d, channels=%d,maxsamples=%d,dstSamplesSize=%d", m_context->dstSamplesLinesize, c->channels, m_context->maxDstNbSamples, m_context->dstSamplesSize);
        }
		return true;
	}
    ...
    //只支持双声道；且每次采样必然是1024个采样点
    //android 层录音需要设定buffersize=4096
	bool CGEVideoEncoderMP4::record(const AudioSampleData& data)
	{
		...
		if(data.data[0] != nullptr)
		{
			...
			int srcNbSamples = m_context->maxDstNbSamples;
...
//                CGE_LOG_INFO("dstNbSamples=%d, srcNbSamples=%d", m_context->maxDstNbSamples, srcNbSamples);
				int ret = swr_convert(m_context->pSwrCtx, m_context->dstSampleData, m_context->maxDstNbSamples, (const uint8_t**)data.data, srcNbSamples);
...
//					break;
				...
				if(m_context->dstSampleDataIndex >= m_context->maxDstNbSamples)
				{
                    ...
                    avcodec_fill_audio_frame(pAudioFrame, audioCodec->channels, audioCodec->sample_fmt, *m_context->dstSampleData, m_context->dstSamplesSize, 0);
					pAudioFrame->data[0] = m_context->dstSampleData[0];
                    pAudioFrame->linesize[0] = m_context->dstSamplesLinesize;
                    pAudioFrame->data[1] = m_context->dstSampleData[0]+m_context->dstSamplesLinesize;
					pAudioFrame->linesize[1] = m_context->dstSamplesLinesize;
