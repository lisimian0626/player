...
#include <chrono>
#include <new>
#include <filters/cgeDataParsingEngine.h>
...
namespace CGE
{
	...
	CGEFrameRecorder::CGEFrameRecorder(void* params) :m_Params(params), m_recordThread(nullptr), m_recordImageThread(nullptr), m_encoder(nullptr), m_offscreenContext(nullptr), m_globalFilter(nullptr), m_TextureId(0)
	{
//        loadTexture();
        m_recordingWork = [&](void* pts){
			...
			if (m_TextureId > 0) {
                CGE_LOG_INFO("%u", m_TextureId);
				glEnable(GL_BLEND);
				glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
				m_cacheDrawer->drawTexture(m_TextureId);
				glDisable(GL_BLEND);
			}
	...
	bool CGEFrameRecorder::endRecording(bool shouldSave)
	{
		...
		m_TextureId = 0;
        m_resultMutex.unlock();
        CGE_LOG_INFO("----glDeleteTextures---%d",m_TextureId);
    ...
    bool CGEFrameRecorder::setLyrics(JNIEnv *env, jclass cls, jobject bmp) {
		CGETextureResult *textureResult = cgeLoadTexture(env, bmp);
		m_resultMutex.lock();
		glDeleteTextures(1, &m_TextureId);
		m_TextureId = textureResult->texID;
		m_resultMutex.unlock();
