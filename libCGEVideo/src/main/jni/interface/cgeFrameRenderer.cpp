...
namespace CGE
{
	...
	bool CGEFrameRenderer::init(int srcWidth, int srcHeight, int dstWidth, int dstHeight)
	{
		...
		if(m_cacheDrawer == nullptr)
			m_cacheDrawer = TextureDrawerExt::create();
		...
		return m_textureDrawer != nullptr && m_textureDrawerExtOES != nullptr
			   && m_frameHandler->initWithRawBufferData(nullptr, dstWidth, dstHeight, CGE_FORMAT_RGBA_INT8, false);
	}
	...
	void CGEFrameRenderer::setFilterWithConfig(CGEConstString config, CGETextureLoadFun texLoadFunc, void* loadArg, bool shouldCleanOld)
	{
		CHECK_RENDERER_STATUS;
		...
		filter->setIntensity(1.0f);
		m_frameHandler->clearImageFilters(shouldCleanOld);
		m_frameHandler->addImageFilter(filter);
	}
	...
	void CGEFrameRenderer::setFilter(CGEImageFilterInterfaceAbstract* filter, bool shouldCleanOld)
	{
		CHECK_RENDERER_STATUS;
